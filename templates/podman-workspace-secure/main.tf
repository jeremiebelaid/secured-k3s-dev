...
resource "coder_agent" "main" {
  ...
  startup_script = <<-EOF
    ...
    echo "=== Secure workspace (coder-secure namespace) ==="
    echo "Selective exfiltration controls are enabled by Tetragon + Cilium egress proxy"
    python --version
    podman --version
...
resource "kubernetes_pod_v1" "workspace" {
  ...
  spec {
    ...
    container {
      ...
      args = ["sh", "-c", coder_agent.main.init_script]
      ...
      env {
        name  = "HTTP_PROXY"
        value = "http://dlp-egress-proxy.coder-secure.svc.cluster.local:3128"
      }
      ...
      env {
        name  = "NO_PROXY"
        value = "localhost,127.0.0.1,.svc,.cluster.local,10.42.0.0/16,10.43.0.0/16,172.26.122.221,172.26.122.221:30080"
      }
